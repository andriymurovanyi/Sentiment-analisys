# Created by hiddencoder at 11.03.2019
...
license_key = 'c07c935d22631c2ad8d622b816a72027'
...
try:
    ...
    topics_response = meaningcloud.TopicsResponse(
        meaningcloud.TopicsRequest(license_key, txt=text, lang='en', topicType='e').sendReq())
    ...
    if topics_response.isSuccessful():
        ...
        if entities:
            ...
            for entity in entities:
                print("\t\t" + topics_response.getTopicForm(entity) + ' --> ' + topics_response.getTypeLastNode(
    ...
    # CLASS API CALL class_response = meaningcloud.ClassResponse(meaningcloud.ClassRequest(license_key, txt=text,
    # model=model).sendReq())

    # SENTIMENT API CALL sentiment_response = meaningcloud.SentimentResponse(meaningcloud.SentimentRequest(
    # license_key, lang='en', txt=text, txtf='plain').sendReq())
